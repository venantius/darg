...
(defmigration add-github-pushes-table
  ...
  ; Desire is to create the message: "$USER pushed $HEAD-MESSAGE and ($SIZE - 1) other commits to $REF"
  (up [] (create
           (table :github_pushes
             (integer :id :auto-inc :primary-key)
...
(defmigration add-github-issues-table
  ; Desire is to create the message: "$USER $ACTIONED issues $TITLE"
  (up [] (create
           (table :github_issues
             (integer :id :auto-inc :primary-key)
             (integer :github_users_id [:refer :github_users :id :on-delete :set-null])
...
(defmigration add-github-pullrequests-table
  (up [] (create 
           (table :github_pullrequests
             (integer :id :auto-inc :primary-key)
             (integer :github_users_id [:refer :github_users :id :on-delete :set-null])
             (integer :github_repos_id [:refer :github_repos :id :on-delete :cascade] :not-null)
             (text :action :not-null)
             ...
             (text :url :not-null)
             (timestamp :timestamp :not-null))))
  (down [] (drop (table :github_pullrequests))))
